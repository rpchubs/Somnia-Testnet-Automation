...
LANG = {
    'vi': {
        'title': 'SEND TOKEN ERC20 - SOMNIA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
    ...
    'en': {
        'title': 'SEND ERC20 TOKEN - SOMNIA TESTNET',
        'info': 'Info',
        'found': 'Found',
        'wallets': 'wallets',
...
def load_addresses(file_path: str = "addressERC20.txt", language: str = 'en') -> list:
    try:
        if not os.path.exists(file_path):
            ...
            with open(file_path, 'w') as f:
                f.write("# Thêm địa chỉ nhận token vào đây...\n")
            return []
...
async def send_token(w3: Web3, private_key: str, wallet_index: int, contract_address: str, destination: str, amount: float, language: str = 'en') -> bool:
...
def send_token_sync(w3: Web3, private_key: str, wallet_index: int, contract_address: str, destination: str, amount: float, language: str) -> bool:
    return asyncio.run(send_token(w3, private_key, wallet_index, contract_address, destination, amount, language))

def run_sendtoken(language: str = 'en'):
    print()
    ...
    if choice == '1':
        ...
        addresses = load_addresses('addressERC20.txt', language)
        if not addresses:
            return
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for i, (line_num, privkey) in enumerate(private_keys, 1):
            ...
            futures.append(
                executor.submit(send_token_sync, w3, privkey, i, contract_address, dest, amount, language)
            )
    ...
    print()
    print_border(LANG[language]['completed'].format(successful=successful_sends, total=total_wallets), Fore.GREEN)
    print()
