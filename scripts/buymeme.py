...
def connect_web3(language: str = 'en'):
    try:
        ...
        if not w3.is_connected():
            print(f"{Fore.RED}  ✖ {LANG[language].get('connect_error','Connection error')}{Style.RESET_ALL}")
            sys.exit(1)
...
def get_amount(language: str = 'en'):
    while True:
        try:
            ...
            if amount <= 0:
                print(f"{Fore.RED}  ✖ {LANG[language]['error']}: Amount must be > 0{Style.RESET_ALL}")
            else:
                return amount
        except ValueError:
            print(f"{Fore.RED}  ✖ {LANG[language]['error']}: Invalid number{Style.RESET_ALL}")

def approve_token(w3: Web3, private_key: str, token_address: str, spender_address: str, amount: float, language: str = 'en'):
...
def buy_token(w3: Web3, private_key: str, token_symbol: str, amount: float, language: str = 'en'):
    account = Account.from_key(private_key)
...
def process_one_wallet(w3: Web3, language: str, token_symbol: str, amount: float,
                       wallet_index: int, total_wallets: int, profile_num: int, private_key: str):
    try:
        account = Account.from_key(private_key)
        ...
        approve_tx = approve_token(w3, private_key, SUSDT_ADDRESS, SPENDER_ADDRESS, amount, language)
        if not approve_tx:
            print_separator()
            return False
        success_buy = buy_token(w3, private_key, token_symbol, amount, language)
        result = True if success_buy else False
...
def run_buymeme(language: str = 'en'):
    print()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for idx, (profile_num, pkey) in enumerate(private_keys, start=1):
            futures.append(
                executor.submit(
                    process_one_wallet,
                    w3, language, token_symbol, amount,
                    idx, total_wallets, profile_num, pkey
                )
            )
